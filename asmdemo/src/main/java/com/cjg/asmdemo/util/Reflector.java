 ...
 * 使用方法：
 * 1.反射类中某个方法并调用：
 *  Reflector.on(factoryClass).method("create", Context.class).call(context);
 *  2.反射类中某个字段并取得值
 *   Reflector.on(ActivityManagerNative.class).field("gDefault").get();
 * 3.反射对象中某个字段并设置值
 * Reflector.with(defaultSingleton).field("mInstance").set(activityManagerProxy);
 * 4.反射某个类并创建对象
 *  Reflector.on("android.databinding.DataBinderMapperProxy").constructor().newInstance();
 *  5.反射某个方法，并且为方法绑定调用对象
