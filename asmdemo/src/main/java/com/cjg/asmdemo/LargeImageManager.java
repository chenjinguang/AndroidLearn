...
import com.cjg.asmdemo.util.ConvertUtils;
import com.cjg.asmdemo.util.ResHelper;
    ...
    private void isShwoAlarm(String url) {
        ...
        if (largeImageInfo.getFileSize() >= LargeImage.getInstance().getFileSizeThreshold() ||
            ...
            mMainHandler.post(new Runnable() {
                @Override
                public void run() {
//                    showDialog(largeImageInfo.get, largeImageInfo.getWidth(), largeImageInfo.getHeight(),
...
//                            largeImageInfo.getTargetHeight());
     ...
     * 创建时间: 2020/4/4 22:05
     */
    public void showDialog(final Drawable drawable, int width, int height, double fileSize, double memorySize, int targetWidth,
                           int targetHeigh) {
...
//        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (!Settings.canDrawOverlays(App.instance)) {
                getOverlayPermission();
                //目前如果没有权限的话，只能返回，否则会报错，这样就可能导致有些警告窗不显示
                return;
            }
        }
        final AlertDialog.Builder builder = new AlertDialog.Builder(App.instance);
        View dialogView = View.inflate(App.instance, R.layout.dialog_custom, null);
...
//        mAlarmInfo.put(url, true);
    ...
    private void setImageUrl(final String url, TextView tvImageUrl) {
        if (TextUtils.isEmpty(url)) {
            ...
            if (!url.startsWith("http") && !url.startsWith("https")) {
                ...
                try {
                    final String resourceName = App.instance.getApplicationContext().getResources().getResourceName(Integer.parseInt(tempUrl));
    ...
    private void copyToClipboard(String resourceName) {
        try {
            ...
            Toast.makeText(App.instance.getApplicationContext(), "复制成功！", Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Toast.makeText(App.instance.getApplicationContext(), "复制失败！", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
